...
export async function snapshot(format: 'svg' | 'png', title: string) {
  ...
  if (format === 'svg') { download(blob, `${title}.svg`); return }
  const url = URL.createObjectURL(blob)
  try {
    const image = new Image(); image.src = url; await image.decode()
    const canvas = document.createElement('canvas'); canvas.width = image.width; canvas.height = image.height
    const ctx = canvas.getContext('2d'); if (!ctx) throw new Error('PNG export is unavailable.')
    ctx.fillStyle = '#fafaf8'; ctx.fillRect(0, 0, canvas.width, canvas.height); ctx.drawImage(image, 0, 0)
    const png = await new Promise<Blob>((resolve, reject) => canvas.toBlob(b => b ? resolve(b) : reject(new Error('Could not export image.')), 'image/png'))
