...
beforeEach(() => { database = new BoardDatabase(`test-${crypto.randomUUID()}`); service = new LibraryService(database) })
...
describe('local library integrity', () => {
  it('persists projects and synchronizes folder membership when moved', async () => {
    ...
    expect((await database.folders.get(folder.id))?.drillIds).toEqual([project.id])
    await service.save({ ...project, folderId: null })
    expect((await database.folders.get(folder.id))?.drillIds).toEqual([])
    expect((await database.projects.get(project.id))?.keyframes[0].tokens).toHaveLength(14)
  })
  it('rejects cycles and leaves the tree unchanged', async () => {
    ...
    await expect(service.updateFolder(parent.id, 'Parent', child.id)).rejects.toThrow('inside itself')
  ...
  it('deletes nested folders and their drills without deleting unrelated drills', async () => {
    ...
    const keep = newProject('Keep'); await service.save(keep)
    await service.deleteFolder(parent.id)
    expect(await database.folders.count()).toBe(0)
    expect((await database.projects.toArray()).map(p => p.id)).toEqual([keep.id])
  })
  it('duplicates independently and retains binary images across database reopen', async () => {
    const asset = await service.uploadAsset(new Blob(['image-data'], { type: 'image/png' }))
    const project = newProject(); project.customAssets.push({ id: asset.id, name: asset.name, blobUri: `asset:${asset.id}` })
    await service.save(project)
    const copy = await service.duplicate(project)
    await service.deleteProject(project.id)
    database.close(); await database.open()
    expect(await (await database.assets.get(asset.id))?.blob.text()).toBe('image-data')
    expect((await database.projects.get(copy.id))?.customAssets[0].id).toBe(asset.id)
    await expect(service.deleteAsset(asset.id)).rejects.toThrow('used in a drill')
  })
  ...
  it('validates upload types and size', async () => {
    await expect(service.uploadAsset(new Blob(['x'], { type: 'text/html' }))).rejects.toThrow('Choose a PNG')
    await expect(service.uploadAsset(new Blob([new Uint8Array(10 * 1024 * 1024 + 1)], { type: 'image/png' }))).rejects.toThrow('smaller than')
  })
})
